...
<div class="grid grid-cols-1 gap-4 md:grid-cols-3">
  {#each stats as stat}
    <div class={`overflow-hidden rounded-2xl border p-4 shadow-sm ${stat.highlight ? "border-primary/30 bg-primary text-white" : "border-border/60 bg-card text-foreground"}`}>
      <div class="flex items-start justify-between gap-3">
        ...
        <div class={`rounded-full p-2 ${stat.highlight ? "bg-white/15" : stat.color === "emerald" ? "bg-emerald-100 text-emerald-600" : stat.color === "rose" ? "bg-rose-100 text-rose-600" : "bg-muted text-muted-foreground"}`}>
          <stat.icon class="h-5 w-5" />
        </div>
      </div>

      <div class="mt-4 flex items-center gap-2 text-xs">
        <span class={`flex items-center gap-1 rounded-full px-2 py-1 ${stat.highlight ? "bg-white/15" : stat.up ? "bg-emerald-100 text-emerald-600" : "bg-rose-100 text-rose-600"}`}>
          {#if stat.up}
