...
<section bind:this={sectionRef} class="relative z-10 -mt-38 h-[200vh]">
    <div class="magicpattern relative h-full bg-fixed bg-top bg-no-repeat">
        ...
        <div class="sticky top-30 mx-auto flex h-[calc(100vh-4rem)] w-full items-center justify-center">
            <div class="relative w-[clamp(502px,35vw,440px)]">
                ...
                <div
                      class="absolute z-40 -left-[clamp(9rem,14vw,14rem)] top-[12%] rounded-xl border border-primary/15 bg-white px-4 py-2 shadow-md transition-all duration-450 ease-out"
                    style={`opacity: ${showLeftTop ? 1 : 0}; transform: translateX(${showLeftTop ? 0 : -24}px);`}
                >
                    Catat jajan harian
                </div>
                <div
                      class="absolute z-40 -left-[clamp(11rem,16vw,16rem)] top-[58%] rounded-xl border border-primary/15 bg-white px-4 py-2 shadow-md transition-all duration-450 ease-out"
                    style={`opacity: ${showLeftBottom ? 1 : 0}; transform: translateX(${showLeftBottom ? 0 : -24}px);`}
                >
                    Budget tetap aman
                </div>

                <div
                      class="absolute z-40 -right-[clamp(10rem,15vw,15rem)] top-[23%] rounded-xl border border-primary/15 bg-white px-4 py-2 shadow-md transition-all duration-450 ease-out"
                    style={`opacity: ${showRightTop ? 1 : 0}; transform: translateX(${showRightTop ? 0 : 24}px);`}
                >
                    Laporan otomatis
                </div>
                <div
                      class="absolute z-40 -right-[clamp(8rem,13vw,13rem)] top-[70%] rounded-xl border border-primary/15 bg-white px-4 py-2 shadow-md transition-all duration-450 ease-out"
                    style={`opacity: ${showRightBottom ? 1 : 0}; transform: translateX(${showRightBottom ? 0 : 24}px);`}
                >
                    Reminder mingguan
                </div>
            </div>
